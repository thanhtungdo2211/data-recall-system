...
class BaseTensor(SimpleClass):
    ...
    def cpu(self):
        ...
        Examples:
            >>> data = torch.tensor([[1, 2, 3], [4, 5, 6]]).cpu()
            >>> base_tensor = BaseTensor(data, orig_shape=(720, 1280))
            >>> cpu_tensor = base_tensor.cpu()
    ...
    def cuda(self):
        ...
        Examples:
            ...
            cuda:0
        """
        return self.__class__(torch.as_tensor(self.data).cpu(), self.orig_shape)
...
class Results(SimpleClass):
    ...
    def _apply(self, fn, *args, **kwargs):
        ...
        This method is internally called by methods like .to(), .cpu(), .cpu(), etc.
    ...
    def cuda(self):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> cuda_results = results[0].cpu()  # Move first result to GPU
            >>> for result in results:
            ...     result_cuda = result.cpu()  # Move each result to GPU
        """
