...
class v8ERDDetectionLoss:
    ...
    def __call__(self, preds, t_preds, batch):
        ...
        # get all distill indexes that are not ignored
        distill_indexes = torch.tensor([i for i in range(self.nc_origin) if i not in self.ignore_distill_indexes]).long()
        ...
        t_pred_scores = (t_pred_scores.permute(0, 2, 1).contiguous())[:, :, distill_indexes] # [1, 8400, nc]
        t_pred_distri = t_pred_distri.permute(0, 2, 1).contiguous() # [1, 8400, 64]
        s_pred_scores = (pred_scores[:, :, :self.nc_origin])[:, :, distill_indexes]
        ...
        for i in range(batch_size):
            ...
            weight_targets = topk_s_pred_scores[i].reshape(-1, distill_indexes.size(0)).detach().sigmoid()
...
class v8ERDSegmentationLoss(v8ERDDetectionLoss):
    def __init__(self, model, add_on_indexes=None, ignore_distill_indexes=None, tal_topk=10, nc_origin=80, t_stride=None):
        super().__init__(model, add_on_indexes, ignore_distill_indexes, tal_topk, nc_origin, t_stride)
