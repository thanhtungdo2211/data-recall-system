...
from task_event_detection.kafka_consumer import consume_from_kafka
from task_event_detection.minio_storage import store_in_minio
from task_event_detection.postgres_writer import save_to_postgres
...
with DAG(
    'event_detection_from_camera_pipeline',
    default_args=default_args,
    description='Object detection events pipeline with YOLOv8, Kafka, MinIO, and PostgreSQL',
    # schedule_interval="0 17 * * *",  # 17:00 UTC = 00:00 GMT+7
    start_date=datetime.now(),  # Bắt đầu từ ngày cụ thể ..
    catchup=False,
    tags=['ai', 'computer-vision', 'yolo', 'kafka', 'minio', 'postgres']
) as dag:
